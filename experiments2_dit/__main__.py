"""experiments2_dit 일괄 실행 진입점 — exp1~exp6를 한 번에 (instruction_v2 §4, t-2 평가).

캐시된 앙상블(`outputs/ensembles_dit_tm2/`)을 읽어 6개 실험 모듈을 순차 실행한다.
각 실험은 독립 프로세스로 실행되어 하나가 실패해도 나머지는 계속된다.

선행: `python -m experiments2_dit.ensemble_inference ...` 로 앙상블 캐시 생성.

실행:
    python -m experiments2_dit
    python -m experiments2_dit --ensemble_dir <dir> --figures_dir <dir> --metrics_dir <dir>
"""
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="experiments2_dit exp1~exp6 일괄 실행")
    ...
    for mod, has_metrics in _EXPERIMENTS:
        cmd = [sys.executable, "-m", f"experiments2_dit.{mod}",
