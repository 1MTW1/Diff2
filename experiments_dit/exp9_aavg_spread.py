...
멤버 간 표준편차). 모든 값은 물리 단위로 역정규화한다. 이웃 프레임(t±1)의 역정규화는
중심 시각이 아니라 t±6h 의 시각별 통계를 사용한다.
...
from .utils import (
    ...
    list_ensemble_files, load_ensemble_npz, load_norm_stats, set_plot_defaults,
)
...
_LEADS = [
    ("t-1", "ensemble_pixel_tm1", "x_tm1_true_pixel", -6),
    ("t",   "ensemble_pixel",     "x_t_true_pixel",    0),
    ("t+1", "ensemble_pixel_tp1", "x_tp1_true_pixel",  6),
]
...
def _collect(ensemble_dir: Path) -> dict:
    ...
    for f in files:
        ...
        for lead, mem_key, gt_key, off_h in _LEADS:
            ...
            mem_d = _denorm_field(mem_norm, lead_time, mean, std)      # (N,C,H,W)
...
def _save_grid(series: dict, out_path: Path) -> None:
    """3×3 grid: 행 = lead (t-1,t,t+1), 열 = 변수 (T,u,v)."""
    fig, axes = plt.subplots(3, 3, figsize=(18, 10), sharex=True)
    for ri, (lead, *_rest) in enumerate(_LEADS):
        for ci, v in enumerate(VARIABLES):
            ax = axes[ri, ci]
            s = series[lead][v]
            ...
            if ci == 0:
                ax.set_ylabel(f"lead {lead}\narea-avg", fontsize=10)
            ax.tick_params(axis="y", labelsize=7)
    for ax in axes[-1, :]:
        _format_time_axis(ax)
    axes[0, 0].legend(fontsize=7, loc="upper right")
    fig.suptitle(
        "Exp9 — Korea-box area-average timeseries  "
        "(GT vs ensemble mean ±1σ)  ·  lead × variable",
        y=0.995, fontsize=13,
    )
...
def run_exp9(
    ...
    _save_grid(series, fig_dir / "exp9_aavg_spread.png")
    ...
    print(f"[exp9] wrote exp9_aavg_spread.png + exp9_stats.json")
    return stats
