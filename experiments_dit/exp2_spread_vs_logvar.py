...
latent 캐시의 채널 0 (LATENT_CH) 에서 σ_ℓ=√exp(ℓ) 와 멤버 ensemble spread 의
per-sample Pearson 상관을 보고, GT(x_t) latent 의 sample 간 공간 상관을 baseline 으로
함께 비교한다.
"""
...
def _save_scatter_examples(
    ...
    for ax, (idx, x, y, r) in zip(axes, items):
        ...
        ax.set_xlabel("σ_ℓ = sqrt(exp(ℓ))")
        ax.set_ylabel("ensemble spread")
        ax.set_title(f"sample #{idx}  r={r:.3f}")
    fig.suptitle("Exp2: σ_ℓ vs ensemble spread (latent ch0)", y=1.02)
    fig.tight_layout()
...
def _load_gt_latent_stack(files: list[Path]) -> np.ndarray:
    """모든 sample의 GT x_t latent 채널 0 → (S, H_z, W_z)."""
    arrs = [load_ensemble_npz(f).x_t_true[LATENT_CH].astype(np.float32)
            for f in files]
...
def _save_comparison_hist(
    ...
    """exp2의 per-sample 상관과 GT sample 간 공간상관을 같은 axis에 overlay."""
    ...
    ax.hist(
        ...
        density=True,
        label=f"GT(x_t) cross-sample baseline  "
    ...
    ax.set_title(
        "Exp2: σ_ℓ↔spread vs GT x_t cross-sample correlation"
    )
...
def run_exp2(
    ...
    for i, f in enumerate(files):
        ...
        sigma_ell = np.sqrt(np.exp(es.log_var[LATENT_CH]))
        spread = _ensemble_spread(es.ensemble)[LATENT_CH]
        x = sigma_ell.flatten()
        y = spread.flatten()
    ...
    # ── GT sample 간 공간 상관 분포 (baseline) ──
    gt_stack = _load_gt_latent_stack(files)
