"""실험 5: composite map (quiver + shading)."""
...
def run_exp5(
    ...
    for k, idx in enumerate(picks, start=1):
        ...
        gt_d = _denorm_field(es.x_t_true, es.time_t, mean, std)         # (C,H,W)
        members_d = _denorm_field(es.ensemble, es.time_t, mean, std)    # (N,C,H,W)
        ...
        spread_norm = es.ensemble.std(axis=0, ddof=1)                    # (C,H,W)
